...
#include "volumeconvertfftreal.h"
...
const std::string VolumeConvertFFTReal::loggerCat_("voreen.base.VolumeConvertFFTReal");

VolumeConvertFFTReal::VolumeConvertFFTReal()
...
Processor* VolumeConvertFFTReal::create() const {
    return new VolumeConvertFFTReal();
}

void VolumeConvertFFTReal::process() {
    ...
    if (enableProcessing_.get() || inport_.hasChanged()) {
        ...
        createFFTSpectrumAndExtractReal();
    }
}

void VolumeConvertFFTReal::createFFTSpectrumAndExtractReal() {
    ...
    if (handle->getRepresentation<VolumeRAM>()) {
        ...
        Volume* v =  VolumeOperatorFFTReal::APPLY_OP(handle);
        outport_.setData(v);
    }
    else {
        outport_.setData(0);
