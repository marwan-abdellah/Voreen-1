...
/// Float Volume
void VolumeCreateBase::fillPlane(VolumeRAM_Float* vds, vec3 center, vec3 normal, uint8_t value) {
    for (size_t voxel_z=0; voxel_z<vds->getDimensions().z; voxel_z++) {
        for (size_t voxel_y=0; voxel_y<vds->getDimensions().y; voxel_y++) {
            for (size_t voxel_x=0; voxel_x<vds->getDimensions().x; voxel_x++) {
                vec3 pos = static_cast<vec3>(tgt::svec3(voxel_x, voxel_y, voxel_z));
...
void VolumeCreateBase::fillCircle(VolumeRAM_Float* vds, vec3 center, float radius, uint8_t value) {
    for (size_t voxel_y = 0; voxel_y < vds->getDimensions().y; ++voxel_y) {
        for (size_t voxel_x = 0; voxel_x < vds->getDimensions().x; ++voxel_x) {
            vec3 diff = center - vec3(static_cast<float>(voxel_x), static_cast<float>(voxel_y), center.z);
...
void VolumeCreateBase::fillOrientedCircle(VolumeRAM_Float* vds, vec3 center, vec3 normal, float radius, uint8_t value) {
    for (size_t voxel_z=0; voxel_z<vds->getDimensions().z; voxel_z++) {
        for (size_t voxel_y=0; voxel_y<vds->getDimensions().y; voxel_y++) {
            for (size_t voxel_x=0; voxel_x<vds->getDimensions().x; voxel_x++) {
                vec3 pos = static_cast<vec3>(tgt::svec3(voxel_x, voxel_y, voxel_z));
...
void VolumeCreateBase::fillSphere(VolumeRAM_Float* vds, vec3 center, float radius, uint8_t value) {
    for (size_t voxel_z=0; voxel_z<vds->getDimensions().z; voxel_z++) {
        for (size_t voxel_y=0; voxel_y<vds->getDimensions().y; voxel_y++) {
            for (size_t voxel_x=0; voxel_x<vds->getDimensions().x; voxel_x++) {
                vec3 diff = center - static_cast<vec3>(tgt::svec3(voxel_x, voxel_y, voxel_z));
...
void VolumeCreateBase::fillEllipsoid(VolumeRAM_Float* vds, vec3 center, vec3 radius, uint8_t value) {
    // ugly code - don't do this at home
    for (size_t voxel_z=0; voxel_z<vds->getDimensions().z; voxel_z++) {
        for (size_t voxel_y=0; voxel_y<vds->getDimensions().y; voxel_y++) {
            for (size_t voxel_x=0; voxel_x<vds->getDimensions().x; voxel_x++) {
...
void VolumeCreateBase::fillBox(VolumeRAM_Float* vds, ivec3 start, ivec3 end, uint8_t value) {
    ivec3 i;
    for (i.x = start.x; i.x < end.x; i.x++) {
        for (i.y = start.y; i.y < end.y; i.y++) {
            for (i.z = start.z; i.z < end.z; i.z++) {
...
void VolumeCreateBase::fillOrientedBox(VolumeRAM_Float* vds, vec3 center, vec3 dir, float lengthA, float lengthB,
    float yStart, float yEnd, uint8_t value)
{
    dir = normalize(dir);
    center.y = 0.0f;
...
void VolumeCreateBase::fillCone(VolumeRAM_Float* vds, vec3 center, vec3 /*dir*/, float /*lengthC*/, float yStart, float yEnd, float topRadius, float bottomRadius, uint8_t value) {
    //dir = normalize(dir);
    //center.y = 0.0f;
    //vec3 dir2 = cross(dir, vec3(0.0f, 1.0f, 0.0f));
    for (size_t voxel_z=0; voxel_z<vds->getDimensions().z; voxel_z++) {
