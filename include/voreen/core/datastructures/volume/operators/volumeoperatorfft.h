...
#ifndef VRN_VOLUMEOPERATORFFT_H
#define VRN_VOLUMEOPERATORFFT_H
...
class VRN_CORE_API VolumeOperatorFFTBase : public UnaryVolumeOperatorBase {
public:
    virtual Volume* apply(const VolumeBase* volume) const = 0;
    virtual ComplexVolume* applyComplex(const VolumeBase* volume) const = 0;
};

// Generic implementation:
template<typename T>
class VolumeOperatorFFTGeneric : public VolumeOperatorFFTBase {
public:
    virtual Volume* apply(const VolumeBase* volume) const;
    virtual ComplexVolume* applyComplex(const VolumeBase* volume) const;
...
template<typename T>
Volume* VolumeOperatorFFTGeneric<T>::apply(const VolumeBase* vh) const {
...
template<typename T>
ComplexVolume* VolumeOperatorFFTGeneric<T>::applyComplex(const VolumeBase* vh) const {
    const VolumeRAM* v = vh->getRepresentation<VolumeRAM>();
    if (!v)
        return 0;
    ...
    VolumeAtomic<T>* realVolumeAtomic = va->clone();
    VolumeAtomic<T>* imaginaryVolumeAtomic = va->clone();
    ...
    std::cout << "Volume Size is : " << volumeSize << std::endl;
    ...
    for (int i = 0; i < volumeSize; i++) {
        realVolumeAtomic->voxel(i) = spectralVolume[i][0];
        imaginaryVolumeAtomic->voxel(i) = spectralVolume[i][1];
        std::cout << realVolumeAtomic->voxel(i) << " ";
    }

    ComplexVolume* complexSpectrum = new ComplexVolume();
    complexSpectrum->real = new Volume(realVolumeAtomic, vh);
    complexSpectrum->imaginary = new Volume(imaginaryVolumeAtomic, vh);

    return complexSpectrum;
}


typedef UniversalUnaryVolumeOperatorGeneric<VolumeOperatorFFTBase> VolumeOperatorFFT;

} // namespace

#endif // VRN_VOLUMEOPERATORFFT_H
