...
#ifndef VRN_VOLUMEOPERATORFFTIMAGINARY_H
#define VRN_VOLUMEOPERATORFFTIMAGINARY_H
...
class VRN_CORE_API VolumeOperatorFFTImaginaryBase : public UnaryVolumeOperatorBase {
public:
...
template<typename T>
class VolumeOperatorFFTImaginaryGeneric : public VolumeOperatorFFTImaginaryBase {
public:
...
template<typename T>
Volume* VolumeOperatorFFTImaginaryGeneric<T>::apply(const VolumeBase* vh) const {
    ...
    for (size_t i = 0; i < volumeSize; i++) {
        output->voxel(i) = spectralVolume[i][1];
    }
...
typedef UniversalUnaryVolumeOperatorGeneric<VolumeOperatorFFTImaginaryBase> VolumeOperatorFFTImaginary;

} // namespace

#endif // VRN_VOLUMEOPERATORFFTIMAGINARY_H
